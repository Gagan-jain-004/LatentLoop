...
function getPollFillWidthClass(percentage: number): string {
  ...
  return (
    ...
    >
      ...
      {poll && poll.options.length > 0 && (
        <div className="mb-4 rounded-2xl border border-cyan-300/60 bg-linear-to-br from-cyan-50/80 to-white p-3 dark:border-cyan-900/70 dark:from-slate-900/80 dark:to-slate-900/50">
          <div className="mb-3 flex items-center justify-between gap-3">
            <p className="wrap-anywhere text-sm font-bold text-slate-800 dark:text-slate-100">📊 {poll.question}</p>
            <span className="shrink-0 rounded-full bg-cyan-100 px-2 py-1 text-[11px] font-semibold text-cyan-800 dark:bg-cyan-900/30 dark:text-cyan-200">
          ...
          <div className="space-y-2">
            {poll.options.map((option) => {
              ...
              const fillClass = getPollFillWidthClass(percentage);
                ...
                <button
                  ...
                  className={`relative w-full overflow-hidden rounded-xl border px-3 py-2 text-left text-sm transition-all disabled:cursor-not-allowed disabled:opacity-80 ${
                    isSelected
                      ...
                      : 'border-sky-200 bg-white/90 text-slate-700 hover:border-cyan-300 dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-200 dark:hover:border-cyan-700'
                  }`}
                >
                  <span className="relative z-10 flex items-center justify-between gap-3">
                    <span className="wrap-anywhere font-medium">{option.text}</span>
                    <span className="shrink-0 text-xs font-semibold text-slate-600 dark:text-slate-300">
                      {hasPollVoted ? `${option.votes} (${percentage}%)` : isSelected ? 'Selected' : 'Tap to vote'}
                    </span>
                  </span>

                  {hasPollVoted && (
                    <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700/60">
                      <div className={`h-full rounded-full bg-cyan-500 transition-all duration-500 ${fillClass}`} />
                    </div>
                  )}
                </button>
              );
          ...
          {!hasPollVoted && (
            <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">Vote once to reveal detailed results.</p>
          )}
        </div>
      )}
